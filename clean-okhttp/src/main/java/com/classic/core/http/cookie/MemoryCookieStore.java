 ...
 */
@SuppressWarnings("unused") public class MemoryCookieStore implements CookieStore {

    private final HashMap<String, List<Cookie>> allCookies = new HashMap<>();

    @Override public void add(HttpUrl url, List<Cookie> cookies) {
    ...
    @Override public List<Cookie> get(HttpUrl uri) {
...
//        }
        ...
        for (Map.Entry<String, List<Cookie>> item : httpUrls) {
            cookies.addAll(item.getValue());
        }
        return cookies;
    }

    @Override public boolean remove(HttpUrl uri, Cookie cookie) {
        List<Cookie> cookies = allCookies.get(uri.host());
